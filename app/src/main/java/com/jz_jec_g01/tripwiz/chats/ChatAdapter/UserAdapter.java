package com.jz_jec_g01.tripwiz.chats.ChatAdapter;
...
public class UserAdapter extends RecyclerView.Adapter<UserAdapter.ViewHolder> {
    ...
    public UserAdapter(Context mContext, List<ChatUser> mChatUsers, boolean ischat){
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        final ChatUser chatUser = mChatUsers.get(position);
        ...
        if (ischat){
            lastMessage(chatUser.getId(), holder.last_msg);
        } else {
            holder.last_msg.setVisibility(View.GONE);
        }

        if (ischat){
            if (chatUser.getStatus().equals("online")){
    ...
    public  class ViewHolder extends RecyclerView.ViewHolder{
        ...
        public ViewHolder(View itemView) {
            ...
            profile_image = itemView.findViewById(R.id.imageViewUser);
